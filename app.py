...
# Translate sinhala to english using google gemini pro -----------------------------------------------------------------------------------------
...
# Check whether ocr text is sinhala or english --------------------------------------------------------------------------------------------------
...
# Function to call the LLM model to extract details
...
if uploaded_file is not None:
    ...
    # Button to get shop details
    if st.button("Get Shop Details"):
        # Call LLM model to extract details
        details = get_shop_details(ocr_text)
        
        # Split the output string into parts
        try:
            shop_name = details.split("Shop Name: ")[1].split("Address: ")[0]
            address = details.split("Address: ")[1].split("Telephone Number: ")[0]
            telephone = details.split("Telephone Number: ")[1]

            # Display the extracted details
            st.write("**Extracted Shop Details**")
            st.write(f"**Shop Name:** {shop_name}")
            st.write(f"**Address:** {address}")
            st.write(f"**Telephone Number:** {telephone}")
        
        except IndexError:
            st.write("Unable to extract shop details properly. Please check the OCR output.")

        # Optionally, clean up the temp image file
